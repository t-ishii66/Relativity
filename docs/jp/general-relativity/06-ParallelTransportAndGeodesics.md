...
ある観測者の時計と物差しに合わせた局所慣性座標で、粒子を観測しよう。時間座標を $x^0=ct$、粒子の三次元の速度を $v^i=dx^i/dt$ とする。この座標では、特殊相対論と同じように、
...
\qquad
U^i=\frac{dx^i}{dt}\frac{dt}{d\tau}=\gamma v^i
$$
...
>
> 一方、$p^\mu=mU^\mu=m\,dx^\mu/d\tau$ は、一般の座標でも成り立つ。「位置をどの座標で表すか」と「何を使って微分するか」は別だからである。座標を変えても、微分に使うのは粒子自身の時計が刻む固有時 $\tau$ である。変わるのは微分される側の座標 $x^\mu$ であり、四元速度と四元運動量の成分は同じ変換を受ける。そのため、両者の関係 $p^\mu=mU^\mu$ は保たれる。
