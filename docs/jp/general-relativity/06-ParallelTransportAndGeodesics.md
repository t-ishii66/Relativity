...
この書き換えを式 (6.1) の第二項に代入すると、式 (6.1) は、
...
と書ける。第一項の $\boldsymbol e_\beta$ にも同じ変換を使い、その項の和の添字 $\beta$ を $\lambda$ と書き換えると、
...
この二階微分の項があるため、座標変換を適切に選べば、ある一点で第二項が第一項をちょうど打ち消すようにできる。したがって、クリストッフェル記号は、その一点ではすべてゼロにできる。ただし、一点の周囲までゼロにできるとは限らない。この違いは、後にリーマン曲率テンソルを考えるときに重要になる。
...
最後の項は、式 (6.2) の第二項と同じ形で、符号だけが反対になっている。
...
クリストッフェル記号は計量から計算される。そのため、重力によって時空の計量が変われば、その影響は、測地線方程式のクリストッフェル記号を通して物体の座標上の運動に現れる。時間も含めた座標について、出発点と初めの速度を与えてこの式を解けば、物体のその後の動きがわかる。

ただし、クリストッフェル記号には座標の取り方による効果も含まれている。実際、平らな平面でも極座標を使えばゼロではなく、曲がった時空でも、ある一点では座標を選んでゼロにできる。したがって、クリストッフェル記号がゼロでないことだけでは、時空が曲がっているとは判断できない。

一点だけでなく、その周囲まで調べたときに残る、座標変換では消せない時空の曲がりは、クリストッフェル記号から作る曲率に現れる。これは第7章で調べる。
...
物体の固有時を $\tau$ とする。第3章と同じく、 $\tau$ は通常の固有時間に $c$ を掛けた、距離の単位を持つ量である。四元速度は、
...
自由落下する物体の運動を、エネルギーと運動量の言葉でも表しておこう。ここでは、質量 $m$ を持つ小さな粒子を考える。
...
である。固有時 $\tau$ は座標の選び方によらないので、座標を変えると、
...
と変換する。これは、上付き添字を一つ持つベクトルの変換則である。

この四元速度に、スカラーである質量と光速の積 $mc$ を掛けて、

$$
\boxed{p^\mu=mcU^\mu}
$$

と定義する。この $p^\mu$ を**四元運動量**と呼ぶ。スカラーを掛けても変換則は変わらないため、四元運動量も四元ベクトルである。

ここで、四元ベクトルはテンソルとは別の種類の量ではない。**四元ベクトルは、時空の四つの成分を持つ、1階のテンソルである。**「四元」の4は成分の数、「1階」の1は添字の数を表す。

### 四つの成分は何を表すか

ある観測者の時計と物差しに合わせた局所慣性座標で、粒子を観測しよう。時間座標を $x^0=w$ とする。秒で測る座標時は $t=w/c$ なので、粒子の三次元の速度は $v^i=dx^i/dt=c\,dx^i/dw$ である。この座標では、特殊相対論と同じように、
...
となる。この局所慣性座標では、四元速度の各成分は無次元である。これに $mc$ を掛けると、四元運動量は、
...
空間の三成分 $p^i=\gamma mv^i$ は、特殊相対論での運動量である。速さが光速より十分小さければ $\gamma\simeq1$ なので、ニュートン力学の $mv^i$ に戻る。

時間成分はエネルギーに対応する。この観測者が測る粒子の全エネルギーは $E=\gamma mc^2$ なので、

$$
p^0=\frac{E}{c}
$$

となる。ここで $E$ は運動エネルギーだけでなく、静止エネルギーも含む。粒子が観測者に対して静止していれば、 $\gamma=1$、 $E=mc^2$ である。
...
と書ける。エネルギーを $c$ で割ることで、四つの成分すべてが運動量と同じ単位になる。

---

Alice「エネルギーと運動量は、別々の量だと思っていたけれど、一つのベクトルになるんだね。」

Bob「見る人の動きが変われば、測るエネルギーも運動量も変わる。その変化を、四つの成分の変換として一緒に扱えるんだ。」

---

> **Tips：四元運動量の成分と測定値**
>
> 四元運動量 $p^\mu$ は1階のテンソルであり、時間成分も空間成分も同じテンソルの変換則に従う。どの成分についても、座標上の成分と、観測者が実際に測る値との対応を考える必要がある。
>
> 観測者の時計と物差しに合わせた局所慣性座標では、四つの成分を「その観測者が測るエネルギーを $c$ で割ったもの」と「その観測者が測る三方向の運動量」として読める。 $(E/c,p^1,p^2,p^3)$ という表記は、この対応を使ったものである。時間成分だけを $E/c$ と書いているが、空間成分も測定される運動量に対応しており、時間成分だけが測定値に関係するわけではない。
>
> 別の観測者に合わせた局所慣性座標へ変換すれば、変換後の四成分は、その新しい観測者が測るエネルギーと運動量に対応する。
>
> 座標は、「いつ、どこで起きたか」を数値で表し、離れた場所の出来事も一つの図の上で扱うために使う。その意味で、座標系は出来事を描く「キャンバス」のようなものである。ただし、その目盛りが、現実の時計や物差しで測った値とそのまま一致するとは限らない。
>
> 一般の座標では、時間軸も空間の軸も、観測者の時計・物差し・運動に合わせてあるとは限らない。そのため、 $p^0$ をそのまま測定される $E/c$ と読むことも、 $p^1,p^2,p^3$ をそのまま測定される三方向の運動量と読むことも、一般にはできない。これは四成分に共通する、座標上の成分と測定値との対応の問題である。
>
> 一方、 $p^\mu=mcU^\mu=mc\,dx^\mu/d\tau$ は、一般の座標でも成り立つ。「位置をどの座標で表すか」と「何を使って微分するか」は別だからである。座標を変えても、微分に使うのは粒子自身の時計が刻む固有時 $\tau$ である。変わるのは微分される側の座標 $x^\mu$ であり、四元速度と四元運動量の成分は同じ変換を受ける。そのため、両者の関係 $p^\mu=mcU^\mu$ は保たれる。
...
である。自由落下では $DU^\mu/D\tau=0$ だから、

$$
\boxed{\frac{Dp^\mu}{D\tau}=0}
$$

となる。つまり、自由落下する粒子の四元運動量は、その測地線に沿って平行移動される。

これは、どの座標でも四つの成分の数値が一定になるという意味ではない。共変微分には、場所による基底の変化を補う接続の項が含まれているからである。

第8章では、粒子一個のエネルギーと運動量から、粒子集団の密度と流束へ進む。そのとき、この四元運動量が出発点になる。
