...
![地球儀の二つの経路と向きの違う矢印を見比べ、目を輝かせるAliceとBob](../../../images/general-relativity/07/globe-discovery.webp)

地球儀の線は運ぶ経路、Bob が手に持つ二本は到着後の向きを比べるための矢印である。二人が見つけた違いを、次の図で詳しく確かめよう。
...
Alice「途中で矢印を回していないのに、最後の向きが違うんだね。」

Bob「同じ場所で比べても違いが残るね。このずれを、式でも追いかけてみよう。」
...
左辺は、テンソルである $\nabla_\nu V^\rho$ 全体をさらに共変微分し、その順序の差を取ったものなので、テンソルである。また、左辺の交換子を展開すると、 $V^\sigma$ の偏微分を含む項はすべて打ち消され、 $V^\sigma$ そのものに比例する項だけが右辺に残る。どのようなベクトル $V^\sigma$ を入れても、右辺は左辺と同じように座標変換しなければならない。そのため、 $V^\sigma$ に掛かっている ${R^\rho}_{\sigma\mu\nu}$ 自体が、一つの上付き添字と三つの下付き添字を持つテンソルとして変換する。
...
したがって、リーマン曲率テンソルを調べれば、微小な閉曲線を一周したときにベクトルがどれだけ、どの方向へずれるかが分かる。この意味で、リーマン曲率テンソルによって、空間や時空がどの方向について、どのように曲がっているかを評価できる。

これで、共変微分の交換子と、小さな閉曲線に沿う平行移動に、同じ曲率が現れることが分かった。

---

Alice「微分の順番の違いが、地球儀で見た矢印のずれにつながった！」

Bob「どの面を回って、どの矢印を運ぶか。その違いまで添字に入っているんだね。」
...
Bob「極座標の式は複雑だったけれど、計算するとちゃんと平面だと分かるんだね。」

Alice「では、球面では何が打ち消されずに残るんだろう？」
...
![球面上の微小な移動を、極角方向と方位角方向に分けた図](../../../images/general-relativity/07/sphere-line-element.png)
...
![図書室で平面と球面の計算結果を見比べ、打ち消し合いの違いを発見して喜ぶAliceとBob](../../../images/general-relativity/07/curvature-discovery-in-library.webp)

左の平面では $-1+1=0$。右の球面では $\cos^2\theta$ の項だけが打ち消され、 $\sin^2\theta$ が残る。二人が指さす計算結果の違いに注目しよう。

これは、球面上でベクトルを小さな閉曲線に沿って平行移動すると、一周後に向きのずれが残ることを表している。

極では $\sin\theta=0$ になる。しかし極では、 $\phi$ がどの値でも同じ一点を表すため、方位角方向を座標として区別できなくなる。つまり、球面がそこで滑らかでなくなるのではなく、 $\theta$ と $\phi$ という座標の付け方が使えなくなるのである。

そのため、 $\phi$ 方向を含むこの成分が極でゼロになっても、球面の曲率そのものが極だけゼロになるわけではない。

## 一点で接続を消しても曲率は残る

第6章の[「クリストッフェル記号はどう変換するか」](./06-ParallelTransportAndGeodesics.md#クリストッフェル記号はどう変換するか)で見たように、座標変換の二階微分を適切に選べば、曲がった時空でも、ある一点で、
...
物理的には、その点の近くで自由落下する座標を選ぶことに対応する。
...
Alice「一つの数になれば読みやすいけれど、それだけでは分からないこともあるんだね。」

Bob「うん。合計がゼロでも、足したものが全部ゼロとは限らない。どの方向にどう曲がっているかを調べるには、元のテンソルの情報が必要なんだ。」
...
次の文書[「物質と曲率を結びつける」](./08-EinsteinEquation.md)では、リッチテンソルと曲率スカラーからアインシュタインテンソルを作り、物質と時空を結ぶアインシュタイン方程式へ進む。
