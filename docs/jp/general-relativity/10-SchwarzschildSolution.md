...
真空では $R_{ww}=R_{rr}=0$ なので、式 (10.2)、(10.3) より、
...
すでに $B=1/A$、$\beta=-\alpha$ と分かったので、式 (10.5) は、
...
残る $R_{wr}$ は、時間の反転 $w\to-w$ で確かめられる。いまの計量は時間に依存せず、$dw\,dr$ の項もないので、この反転で変わらない。一方、$R_{wr}$ は時間の添字を一つだけ持つため、変換すると符号が反転する。
...
$$
g_{ww}\simeq-\left(1+\frac{2\Phi}{c^2}\right)
$$

となることを導いた。$\Phi$ は単位質量あたりの重力ポテンシャルである。
...
ニュートン近似を使ったのは、積分定数の意味を決めるためである。式 (10.6) 自体は、近似せずに真空の方程式を解いて得ている。
...
をシュヴァルツシルト半径と呼ぶ。求めた計量は、

$$
\boxed{
\begin{aligned}
ds^2={}&-\left(1-\frac{r_{\mathrm s}}r\right)dw^2
...
この章で導いたのは、$r>r_{\mathrm s}$ にある静的な真空領域の形である。星の外側を扱うなら、さらに星の表面より外に限る。星の内部には物質があるので、同じ真空の方程式は使えない。
...
$$
d\tau=\sqrt{1-\frac{r_{\mathrm s}}r}\,dt,
\qquad
d\ell=\frac{|dr|}{\sqrt{1-r_{\mathrm s}/r}}.
$$
...
次章では、座標を時空の出来事に付ける目印として捉え直し、局所的な時計や物差しによる測定との対応を整理しよう。
